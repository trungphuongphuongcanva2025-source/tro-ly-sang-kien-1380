...
app.post("/api/generate-initiative", async (req, res) => {
  ...
  try {
    const systemInstruction = `Bạn là "Trợ lý học thuật và Sáng tạo Sáng kiến CHỐNG AI MẠNH, Chuyên gia Soạn thảo Văn bản Hành chính Cấp cao" — Chuyên gia đồng hành hỗ trợ giáo viên viết sáng kiến kinh nghiệm thực tế sâu sắc.

QUY TẮC ĐỊNH DẠNG HÀNH CHÍNH CHUẨN (TUÂN THỦ TUYỆT ĐỐI THEO YÊU CẦU NUÔI):
1. PHÂN CẤP TIÊU ĐỀ:
- Cấp 1 (Phần chính): Các phần chính bắt buộc viết HOA, in đậm, đứng riêng một dòng dạng: **A. MỞ ĐẦU**, **B. NỘI DUNG**, **C. KẾT LUẬN**, **D. TÀI LIỆU THAM KHẢO, MỤC LỤC**.
- Cấp 2: Các mục nhỏ (1., 2., 3...) bắt buộc in đậm, đứng riêng một dòng dạng:
  **1. Tên sáng kiến**
...
- Cấp 3: Các mục con dạng hai chữ số và ba chữ số (2.1., 2.2., 1.1., 1.2., 2.1., 2.2., 3.1., 3.2., 3.2.1., 3.2.2...) bắt buộc in đậm, đứng riêng một dòng dạng:
...
- Cấp 4: Các ý nhỏ trong giải pháp bám sát chữ số thường (a, b, c, d) in đậm, thụt lề dạng:
...
- Phần Cơ sở lý luận: Phải diễn đạt lại hoàn toàn bằng ngôn từ chiêm nghiệm, cách suy nghĩ của giáo viên thực tế (paraphrase toàn bộ lý luận), tránh sao chép khô khan nguyên mẫu thuật ngữ giáo điều của Bộ GD&ĐT hay các thông tư. (Ví dụ: thay vị viết dập khuôn "phát triển phẩm chất và năng lực", hãy viết cụ thể "giúp học sinh tự tin diễn đạt, có thói quen chủ động tìm hiểu trước câu chuyện đời thường và biết cách chia sẻ giúp đỡ bạn bè cùng tiến bộ").
...
QUY CƠ CẤP TRÌNH BÀY BẮT BUỘC TRONG TOÀN BÀI:
...
**2. Sự cần thiết, mục đích của việc thực hiện sáng kiến**
**2.1. Trình bày sự cần thiết**
- [Trình bày súc tích lý do, bối cảnh thực tiễn đổi mới dạy học theo chương trình GDPT 2018 tại địa phương ${prov} và trường ${sName} dẫn đến việc cần thiết phải thực hiện sáng kiến này. Nêu bật những bất cập hiện tại khi chưa áp dụng, cực kỳ súc tích, khoảng 150 - 200 ký tự].

**2.2. Mục đích của việc thực hiện sáng kiến**
- [Trình bày cực kỳ súc tích, rõ ràng và trực diện mục đích thực tiễn sư phạm cần đạt được khi thực hiện sáng kiến, dài khoảng 80-100 ký tự].
...
c) Cách thực hiện: [Cách thực hiện súc tích, ngắn gọn xưng 'tôi'. Lồng ghép ví dụ dạy học cụ thể ngắn gọn bắt đầu bằng: 'Ví dụ: Khi dạy bài [Tên bài học] trong sách giáo khoa [Bộ môn - Lớp] [Bộ sách]...'. Dùng các gạch đầu dòng (-) trong ví dụ.]
...
**4. Kết quả mang lại**
- [Trình bày kết quả đạt được sau khi tiến hành nghiên cứu, thực nghiệm các giải pháp mới.]
...
- [Trình bày tóm tắt mục lục các phần A, B, C, D chính xác theo cấu trúc trên]`;
    ...
    const userPrompt = `Hãy viết một bản sáng kiến kinh nghiệm giáo dục toàn diện chính thức hoàn chỉnh dựa trên các thông số sau:
- Tên sáng kiến được chọn: "${title}"
- Cấp đăng ký: "${level}"
- Cấp học giáo viên hoạt động: "${sLevel}"
- Trường giảng dạy: "${sName}"
- Tỉnh / Thành phố: "${prov}"
- Ý tưởng thô đầu vào: "${inputData.rawIdea}"
- Môn học/Lĩnh vực: "${inputData.subject}"
- Nhóm đối tượng học sinh: "${inputData.targetStudents}"
- Vấn đề cốt lõi: "${inputData.coreProblem}"
- Mục tiêu định hướng: "${inputData.goal}"
- Bộ sách giáo khoa chính: "${inputData.textbook || "Kết nối tri thức với cuộc sống"}"

Hãy viết trực tiếp nội dung sáng kiến chi tiết từ phần A đến phần D, bảo đảm tổng độ dài BẮT BUỘC nằm trong khoảng từ 2000 đến 2200 ký tự (khoảng 350 - 450 từ/chữ) tiếng Việt. Viết cô đọng, súc tích nhưng đầy đủ các phần cốt lõi theo quy trình chuẩn, giảm tối đa dấu vết AI.`;
    ...
    res.json({
      title: title,
      level: level,
      content: response.text || "Lỗi tạo nội dung từ AI. Vui lòng thử lại.",
    });
  } catch (error: any) {
...
- ĐIỀU CHỈNH và TINH GỌN toàn thể bộ sáng kiến để đạt độ dài BẮT BUỘC nằm trong khoảng từ 2000 đến 2200 ký tự (khoảng 350 - 450 từ/chữ) tiếng Việt hoàn chỉnh tuyệt đối. Hãy viết cực kỳ cô đọng, loại bỏ các diễn giải dông dài nhưng giữ nguyên cấu trúc chuẩn hành chính.
...
QUY CƠ CẤP TRÌNH BÀY BẮT BUỘC TRONG TOÀN BÀI (Dung lượng 2000 - 2200 ký tự):
...
Sau khi đáp ứng, hãy ĐIỀU CHỈNH cô đọng và tinh gọn tất cả các phần để đưa tổng độ dài toàn bộ sáng kiến (gồm tất cả các phần) đạt ngưỡng BẮT BUỘC từ 2000 đến 2200 ký tự (khoảng 350 - 450 từ/chữ) tiếng Việt hoàn chỉnh tuyệt đối.
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            content: {
              type: Type.STRING,
              description: "Bản sáng kiến hoàn chỉnh đã được cập nhật tinh chỉnh, có tổng dung lượng toàn bài đạt từ 2000 đến 2200 ký tự tiếng Việt."
            },
            displayContent: {
              type: Type.STRING,
